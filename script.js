...
function mkCatBtn(label, val, cnt, active) {
  ...
  b.addEventListener('click', () => { activeFilter = val; updateFilters(); renderList(); });
  return b;
}
...
function buildCard(a, i) {
  const sancs = (a.sanctions || []).map(s => {
    ...
    if (s.type === 'autre') {
      text = esc(s.valeur || '—');
    } else if (s.type === 'amende') {
      text = `${def.label} · <strong>${s.valeur} ${esc(s.unite)}</strong>`;
    } else {
      text = `${def.label} · <strong>${s.valeur} ${esc(s.unite)}</strong>`;
    }
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') q('searchInput').value = '' || renderList();
});
